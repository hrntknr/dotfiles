...
const notificationTitle = (sessionID) =>
  `opencode: ${sessionTitles.get(sessionID) ?? "opencode"}`;
    ...
    event: async ({ event }) => {
      if (event.type === "session.idle") {
        ...
        try {
          ...
          const message =
            lastAssistantTexts.get(sessionID) ?? "作業が完了しました";
      ...
      if (event.type === "message.updated") {
        if (
          ...
          event.properties.info?.type === "assistant"
        ) {
      ...
      if (event.type === "message.part.updated") {
        ...
        if (
          part.type === "text" &&
          assistantMessageIDs.get(sessionID)?.has(part.messageID)
        ) {
      ...
      if (event.type === "permission.asked") {
        ...
        const detail = patterns.length
          ...
          : permission;
        await notify($, {
          title: notificationTitle(sessionID),
          group: sessionID,
          message: `権限確認待ちです\n${truncate(detail)}`,
        });
      }
